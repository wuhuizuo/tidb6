...
func TestIndexLookupMergeJoinHang(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/IndexMergeJoinMockOOM", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/IndexMergeJoinMockOOM"))
	}()
...
func TestIssue18068(t *testing.T) {
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/testIssue18068", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/testIssue18068"))
	}()
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
