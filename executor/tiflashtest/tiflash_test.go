...
import (
	...
	"github.com/wuhuizuo/tidb6/util/israce"
...
// to fix https://github.com/wuhuizuo/tidb6/issues/27952
...
func TestMppExecution(t *testing.T) {
	...
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks", `return(3)`)
	...
	failpoint.Disable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks")
...
func TestPartitionTable(t *testing.T) {
	...
	tk.MustExec("insert into t values(3,0)")
	tk.MustExec("insert into t values(4,0)")
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkUseMPP", `return(true)`)
	tk.MustExec("set @@session.tidb_isolation_read_engines=\"tiflash\"")
	tk.MustExec("set @@session.tidb_allow_mpp=ON")
	// mock executor does not support use outer table as build side for outer join, so need to
	// force the inner table as build side
	tk.MustExec("set tidb_opt_mpp_outer_join_fixed_build_side=1")
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks", `return(2)`)
	tk.MustQuery("select count(*) from t").Check(testkit.Rows("4"))
	failpoint.Disable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks")
	tk.MustExec("set @@session.tidb_partition_prune_mode='static-only'")
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkUseMPP", `return(false)`)
	tk.MustQuery("select count(*) from t").Check(testkit.Rows("4"))
	...
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks", `return(4)`)
	...
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks", `return(5)`)
	...
	failpoint.Disable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks")
	failpoint.Disable("github.com/wuhuizuo/tidb6/executor/checkUseMPP")
}
...
func TestDispatchTaskRetry(t *testing.T) {
	...
	require.NoError(t, err)
	tk.MustExec("set @@session.tidb_enforce_mpp=ON")
	require.Nil(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppDispatchTimeout", "3*return(true)"))
	tk.MustQuery("select count(*) from t").Check(testkit.Rows("4"))
	require.Nil(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppDispatchTimeout"))
}

func TestCancelMppTasks(t *testing.T) {
	var hang = "github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppRecvHang"
	store := testkit.CreateMockStore(t, withMockTiFlash(2))
	tk := testkit.NewTestKit(t, store)
	tk.MustExec("use test")
	tk.MustExec("drop table if exists t")
...
func TestMppGoroutinesExitFromErrors(t *testing.T) {
	...
	var mppNonRootTaskError = "github.com/wuhuizuo/tidb6/store/copr/mppNonRootTaskError"
...
func TestMppUnionAll(t *testing.T) {
	...
	failpoint.Enable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks", `return(6)`)
	tk.MustQuery("select count(*) from (select * from x1 union all select * from x2 union all select * from x3) x join (select * from x1 union all select * from x2 union all select * from x3) y on x.a = y.b").Check(testkit.Rows("29"))
	failpoint.Disable("github.com/wuhuizuo/tidb6/executor/checkTotalMPPTasks")
...
func TestTiflashPartitionTableScan(t *testing.T) {
	...
	wg := sync.WaitGroup{}
	wg.Add(1)
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/rpcServerBusy", `return(true)`))
	go func() {
		time.Sleep(100 * time.Millisecond)
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/rpcServerBusy"))
		wg.Done()
	}()
	tk.MustExec("set @@session.tidb_allow_mpp=ON;")
	tk.MustQuery("select count(*) from t where a < 12;").Check(testkit.Rows("2"))
...
func TestMPPMemoryTracker(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/copr/testMPPOOMPanic", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/copr/testMPPOOMPanic"))
	}()
