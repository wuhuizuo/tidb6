...
import (
	...
	"github.com/wuhuizuo/tidb6/executor"
	...
	"github.com/wuhuizuo/tidb6/statistics/handle"
	...
	"github.com/wuhuizuo/tidb6/util/codec"
...
func TestFailedAnalyzeRequest(t *testing.T) {
	...
	tk.MustExec("set @@tidb_analyze_version = 1")
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/buildStatsFromResult", `return(true)`))
	_, err := tk.Exec("analyze table t")
	require.NotNil(t, err)
	require.Equal(t, "mock buildStatsFromResult error", err.Error())
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/buildStatsFromResult"))
}
...
func testSnapshotAnalyzeAndMaxTSAnalyzeHelper(analyzeSnapshot bool) func(t *testing.T) {
	return func(t *testing.T) {
		...
		tk.MustExec("commit")
		require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/injectAnalyzeSnapshot", fmt.Sprintf("return(%d)", startTS1)))
		tk.MustExec("analyze table t")
		...
		require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/injectAnalyzeSnapshot", fmt.Sprintf("return(%d)", startTS2)))
		tk.MustExec("analyze table t")
		rows = tk.MustQuery(fmt.Sprintf("select count, snapshot from mysql.stats_meta where table_id = %d", tid)).Rows()
		require.Len(t, rows, 1)
		require.Equal(t, "6", rows[0][0])
		...
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/injectAnalyzeSnapshot"))
	}
}
...
func TestAnalyzeSamplingWorkPanic(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeSamplingBuildWorkerPanic", "return(1)"))
	err := tk.ExecToErr("analyze table t")
	require.NotNil(t, err)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeSamplingBuildWorkerPanic"))
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeSamplingMergeWorkerPanic", "return(1)"))
	err = tk.ExecToErr("analyze table t")
	require.NotNil(t, err)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeSamplingMergeWorkerPanic"))
}

func TestSmallTableAnalyzeV2(t *testing.T) {
	failpoint.Enable("github.com/wuhuizuo/tidb6/planner/core/forceDynamicPrune", `return(true)`)
	defer failpoint.Disable("github.com/wuhuizuo/tidb6/planner/core/forceDynamicPrune")
	store := testkit.CreateMockStore(t)

	tk := testkit.NewTestKit(t, store)
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/calcSampleRateByStorageCount", "return(1)"))
	tk.MustExec("use test")
	tk.MustExec("set @@session.tidb_analyze_version = 2")
...
create table small_table_inject_pd_with_partition(
	...
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/calcSampleRateByStorageCount"))
}
...
func testKillAutoAnalyze(t *testing.T, ver int) {
	...
	for _, status := range []string{
		...
		func() {
			...
			mockAnalyzeStatus := "github.com/wuhuizuo/tidb6/executor/mockKill" + strings.Title(status)
			if status == "running" {
			...
			if status == "pending" || status == "running" {
				mockSlowAnalyze := "github.com/wuhuizuo/tidb6/executor/mockSlowAnalyzeV" + strconv.Itoa(ver)
...
func TestKillAutoAnalyzeIndex(t *testing.T) {
	...
	for _, status := range []string{"pending", "running", "finished"} {
		func() {
			...
			if status == "pending" || status == "running" {
				require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/mockSlowAnalyzeIndex", "return"))
				defer func() {
					require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/mockSlowAnalyzeIndex"))
				}()
			}
...
func TestAutoAnalyzeAwareGlobalVariableChange(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/injectAnalyzeSnapshot", fmt.Sprintf("return(%d)", startTS)))
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/injectBaseCount", "return(3)"))
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/injectBaseModifyCount", "return(0)"))
	...
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/injectAnalyzeSnapshot"))
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/injectBaseCount"))
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/injectBaseModifyCount"))
}
...
func TestGlobalMemoryControlForAnalyze(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/util/memory/ReadMemStats", `return(536870912)`))
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeMergeWorkerSlowConsume", `return(100)`))
	_, err := tk0.Exec(sql)
	require.True(t, strings.Contains(err.Error(), memory.PanicMemoryExceedWarnMsg+memory.WarnMsgSuffixForInstance))
	runtime.GC()
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/util/memory/ReadMemStats"))
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeMergeWorkerSlowConsume"))
	tk0.MustExec(sql)
}
...
func TestGlobalMemoryControlForPrepareAnalyze(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/util/memory/ReadMemStats", `return(536870912)`)) // 512MB
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/executor/mockAnalyzeMergeWorkerSlowConsume", `return(100)`))
