...
func TestAddIndexIngestPanicOnCopRead(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/ddl/MockCopSenderPanic", "return(true)"))
	...
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/ddl/MockCopSenderPanic"))
	rows := tk.MustQuery("admin show ddl jobs 1;").Rows()
	require.Len(t, rows, 1)
	jobTp := rows[0][3].(string)
