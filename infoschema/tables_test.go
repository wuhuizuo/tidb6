...
import (
	...
	"github.com/wuhuizuo/tidb6/util/gctuner"
...
func TestStmtSummaryTable(t *testing.T) {
	...
	// Test SELECT.
	const failpointName = "github.com/wuhuizuo/tidb6/planner/core/mockPlanRowCount"
...
func TestSimpleStmtSummaryEvictedCount(t *testing.T) {
	...
	tk.MustExec("set @@global.tidb_enable_stmt_summary=1")
	historySize := 24
	fpPath := "github.com/wuhuizuo/tidb6/util/stmtsummary/mockTimeForStatementsSummary"
...
func TestAttributes(t *testing.T) {
	...
	fpName := "github.com/wuhuizuo/tidb6/executor/mockOutputOfAttributes"
	tk := newTestKitWithRoot(t, store)
...
// TestTableConstraintsContainForeignKeys TiDB Issue: https://github.com/wuhuizuo/tidb6/issues/28918
...
func TestMemoryUsageAndOpsHistory(t *testing.T) {
	store := testkit.CreateMockStore(t)
	tk := testkit.NewTestKit(t, store)
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/util/gctuner/testMemoryLimitTuner", "return(true)"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/util/gctuner/testMemoryLimitTuner"))
	}()
