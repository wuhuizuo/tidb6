...
- Tracking Issue: [Tracking issue for moving parser back to TiDB](https://github.com/wuhuizuo/tidb6/issues/28257)
...
The original PR moving out parser is [tidb#7923: Move TiDB parser to a separate repository](https://github.com/wuhuizuo/tidb6/issues/7923). When we migrated to go module 111, @tiancaiamao decided to do so, because:
...
4. We can create wrapper packages for pingcap/parser, which re-exports things from `pingcap/tidb/parser`. I mean something like `import ( . github.com/wuhuizuo/tidb6/parser/xxx)`, create a dummy function will eliminate the error of `not used import`. All code of pingcap/parser will be removed in this step. This will delevery new updates without the need of migrating importing paths.
