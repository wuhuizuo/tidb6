...
function build_importer()
{
    ...
    rm -rf $importer
    GO111MODULE=on go build -o $importer github.com/wuhuizuo/tidb6/cmd/importer
}
...
function build_portgenerator()
{
    ...
    rm -rf $portgenerator
    GO111MODULE=on go build -o $portgenerator github.com/wuhuizuo/tidb6/cmd/portgenerator
}
...
function build_tidb_server()
{
    ...
    if [ "${TIDB_TEST_STORE_NAME}" = "tikv" ]; then
        GO111MODULE=on go build -o $tidb_server github.com/wuhuizuo/tidb6/tidb-server
    else
        GO111MODULE=on go build -race -o $tidb_server github.com/wuhuizuo/tidb6/tidb-server
    fi
}
