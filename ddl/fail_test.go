...
func TestFailBeforeDecodeArgs(t *testing.T) {
	...
	tc.OnJobRunBeforeExported = func(job *model.Job) {
		...
		if job.SchemaState == model.StateWriteOnly {
			...
			if first {
				require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/ddl/errorBeforeDecodeArgs", `return(true)`))
				first = false
			} else {
				require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/ddl/errorBeforeDecodeArgs"))
			}
		}
	}
	d.SetHook(tc)
