...
func TestTableError(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/ddl/mockModifyJobSchemaId", `return(-1)`))
	err := tk.ExecToErr("drop table testDrop")
	require.Error(t, err)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/ddl/mockModifyJobSchemaId"))

	// Table ID is wrong, so dropping table is failed.
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/ddl/MockModifyJobTableId", `return(-1)`))
	err = tk.ExecToErr("drop table testDrop")
	require.Error(t, err)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/ddl/MockModifyJobTableId"))

	// Args is wrong, so creating table is failed.
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/ddl/MockModifyJobArg", `return(true)`))
	err = tk.ExecToErr("create table test.t1(a int)")
	require.Error(t, err)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/ddl/MockModifyJobArg"))
