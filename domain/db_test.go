...
func TestAbnormalSessionPool(t *testing.T) {
	...
	pool := domain.SysSessionPool()
	failpoint.Enable("github.com/wuhuizuo/tidb6/domain/mockSessionPoolReturnError", "return")
	se, err := pool.Get()
	require.Error(t, err)
	failpoint.Disable("github.com/wuhuizuo/tidb6/domain/mockSessionPoolReturnError")
	require.Equal(t, svr.InternalSessionExists(se), false)
}
