...
func TestRegionsFromMeta(t *testing.T) {
	...
	// test no panic
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/errGetRegionByIDEmpty", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/errGetRegionByIDEmpty"))
	}()
...
func TestFailpointHandler(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/enableTestAPI", "return"))
	defer func() { require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/enableTestAPI")) }()
	ts.startServer(t)
	resp, err = ts.fetchStatus("/fail/")
	require.NoError(t, err)
	require.Equal(t, http.StatusOK, resp.StatusCode)
	...
	require.NoError(t, err)
	require.True(t, strings.Contains(string(b), "github.com/wuhuizuo/tidb6/server/enableTestAPI=return"))
	require.NoError(t, resp.Body.Close())
}
