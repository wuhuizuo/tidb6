...
import (
	...
	"github.com/wuhuizuo/tidb6/util/arena"
...
func TestConnExecutionTimeout(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/FakeClientConn", "return(1)"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/FakeClientConn"))
	}()
	tk := testkit.NewTestKit(t, store)
...
func TestTiFlashFallback(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/copr/ReduceCopNextMaxBackoff", `return(true)`))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/copr/ReduceCopNextMaxBackoff"))
	}()

	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/BatchCopRpcErrtiflash0", "return(\"tiflash0\")"))
	...
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/BatchCopRpcErrtiflash0"))

	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/fetchNextErr", "return(\"firstNext\")"))
	// test COM_STMT_EXECUTE (cursor mode)
	tk.MustExec("set @@tidb_allow_fallback_to_tikv='tiflash'")
	require.NoError(t, cc.handleStmtExecute(ctx, []byte{0x1, 0x0, 0x0, 0x0, 0x1, 0x1, 0x0, 0x0, 0x0}))
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/fetchNextErr"))

	// test that TiDB would not retry if the first execution already sends data to client
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/fetchNextErr", "return(\"secondNext\")"))
	tk.MustExec("set @@tidb_allow_fallback_to_tikv='tiflash'")
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/batchCopRecvTimeout", "return(true)"))
	testFallbackWork(t, tk, cc, "select count(*) from t")
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/batchCopRecvTimeout"))

	// TiFlash MPP query (MPP + streaming)
	tk.MustExec("set @@tidb_allow_batch_cop=0; set @@tidb_allow_mpp=1;")

	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppDispatchTimeout", "return(true)"))
	testFallbackWork(t, tk, cc, "select * from t t1 join t t2 on t1.a = t2.a")
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppDispatchTimeout"))

	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppRecvTimeout", "return(-1)"))
	testFallbackWork(t, tk, cc, "select * from t t1 join t t2 on t1.a = t2.a")
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/mppRecvTimeout"))

	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/establishMppConnectionErr", "return(true)"))
	testFallbackWork(t, tk, cc, "select * from t t1 join t t2 on t1.a = t2.a")
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/store/mockstore/unistore/establishMppConnectionErr"))
...
// For issue https://github.com/wuhuizuo/tidb6/issues/25069
...
func TestHandleAuthPlugin(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/FakeAuthSwitch", "return(1)"))
	cc = &clientConn{
		connectionID: 1,
		alloc:        arena.NewAllocator(1024),
		chunkAlloc:   chunk.NewAllocator(),
	...
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/FakeAuthSwitch"))
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/FakeUser", "return(\"mysql_native_password\")"))
	...
	err = cc.handleAuthPlugin(ctx, &resp)
	require.NoError(t, err)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/FakeUser"))

	// === Target account has caching_sha2_password ===
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/FakeUser", "return(\"caching_sha2_password\")"))
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/FakeUser", "return(\"tidb_sm3_password\")"))
...
func TestChangeUserAuth(t *testing.T) {
	...
	data = append(data, 0)
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/server/ChangeUserAuthSwitch", fmt.Sprintf("return(\"%s\")", t.Name())))
	err = cc.handleChangeUser(ctx, data)
	require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/server/ChangeUserAuthSwitch"))
...
func TestMaxAllowedPacket(t *testing.T) {
	// Test cases from issue 31422: https://github.com/wuhuizuo/tidb6/issues/31422
