...
func TestNonTransactionalDMLErrorMessage(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/session/batchDMLError", `return(true)`))
	defer failpoint.Disable("github.com/wuhuizuo/tidb6/session/batchDMLError")
	...
	require.NoError(
		t, failpoint.Enable("github.com/wuhuizuo/tidb6/session/batchDMLError", `1*return(false)->return(true)`),
	)
	err = tk.ExecToErr("batch on a limit 3 insert into t1 select * from t")
