...
# See https://github.com/wuhuizuo/tidb6/pull/30692.
...
ifeq ("$(pkg)", "")
	...
	@echo "Running unit test for github.com/wuhuizuo/tidb6/$(pkg)"
	@export log_level=fatal; export TZ='Asia/Shanghai'; \
	$(GOTEST) -ldflags '$(TEST_LDFLAGS)' -cover github.com/wuhuizuo/tidb6/$(pkg) || { $(FAILPOINT_DISABLE); exit 1; }
endif
	@$(FAILPOINT_DISABLE)
...
bench-daily:
	go test github.com/wuhuizuo/tidb6/session -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/executor -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/executor/splittest -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/tablecodec -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/expression -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/util/rowcodec -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/util/codec -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/distsql -run TestBenchDaily -bench Ignore --outfile bench_daily.json
	go test github.com/wuhuizuo/tidb6/util/benchdaily -run TestBenchDaily -bench Ignore \
...
build_for_br_integration_test:
	...
	($(GOTEST) -c -cover -covermode=count \
		-coverpkg=github.com/wuhuizuo/tidb6/br/... \
		-o $(BR_BIN).test \
		github.com/wuhuizuo/tidb6/br/cmd/br && \
	$(GOTEST) -c -cover -covermode=count \
		-coverpkg=github.com/wuhuizuo/tidb6/br/... \
		-o $(LIGHTNING_BIN).test \
		github.com/wuhuizuo/tidb6/br/cmd/tidb-lightning && \
	$(GOTEST) -c -cover -covermode=count \
		-coverpkg=github.com/wuhuizuo/tidb6/br/... \
		-o $(LIGHTNING_CTL_BIN).test \
		github.com/wuhuizuo/tidb6/br/cmd/tidb-lightning-ctl && \
...
data_parsers: tools/bin/vfsgendev br/pkg/lightning/mydump/parser_generated.go br_web
	...
	tools/bin/vfsgendev -source='"github.com/wuhuizuo/tidb6/br/pkg/lightning/web".Res' && mv res_vfsdata.go br/pkg/lightning/web/
