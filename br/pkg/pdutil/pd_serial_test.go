...
func TestPDRequestRetry(t *testing.T) {
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/br/pkg/pdutil/FastRetry", "return(true)"))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/br/pkg/pdutil/FastRetry"))
	}()
	...
	require.NoError(t, failpoint.Enable("github.com/wuhuizuo/tidb6/br/pkg/pdutil/InjectClosed",
		fmt.Sprintf("return(%d)", 0)))
	defer func() {
		require.NoError(t, failpoint.Disable("github.com/wuhuizuo/tidb6/br/pkg/pdutil/InjectClosed"))
	}()
	ts = httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		w.WriteHeader(http.StatusOK)
	}))
