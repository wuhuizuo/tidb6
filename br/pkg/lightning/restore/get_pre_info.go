...
import (
	...
	_ "github.com/wuhuizuo/tidb6/planner/core" // to setup expression.EvalAstExpr. Otherwise we cannot parse the default value
...
func (p *PreRestoreInfoGetterImpl) getTableStructuresByFileMeta(ctx context.Context, dbSrcFileMeta *mydump.MDDatabaseMeta, getPreInfoCfg *ropts.GetPreInfoConfig) ([]*model.TableInfo, error) {
	...
	failpoint.Inject(
		...
		func(v failpoint.Value) {
			...
			//nolint: errcheck
			failpoint.Enable("github.com/wuhuizuo/tidb6/br/pkg/lightning/backend/tidb/FetchRemoteTableModels_BeforeFetchTableAutoIDInfos", fmt.Sprintf("sleep(%d)", sleepMilliSeconds))
		},
	)
