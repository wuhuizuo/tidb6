...
export GO_FAILPOINTS='github.com/wuhuizuo/tidb6/br/pkg/lightning/backend/local/FailIngestMeta=1*return("notleader");github.com/wuhuizuo/tidb6/br/pkg/lightning/backend/local/failToSplit=2*return("")'
...
export GO_FAILPOINTS='github.com/wuhuizuo/tidb6/br/pkg/lightning/backend/local/FailIngestMeta=2*return("epochnotmatch")'
...
set +e
export GO_FAILPOINTS='github.com/wuhuizuo/tidb6/br/pkg/lightning/restore/FailIfStatusBecomes=return(90);'
...
for ckpt in mysql file; do
  ...
  set +e
  export GO_FAILPOINTS="github.com/wuhuizuo/tidb6/br/pkg/lightning/restore/FailAfterWriteRows=return"
  ...
  rm -f "/tmp/tidb_lightning_checkpoint_local_backend_test.pb"
  set +e
  export GO_FAILPOINTS="github.com/wuhuizuo/tidb6/br/pkg/lightning/restore/LocalBackendSaveCheckpoint=return;github.com/wuhuizuo/tidb6/br/pkg/lightning/restore/FailIfImportedChunk=return"
  run_lightning --backend local --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "tests/$TEST_NAME/$ckpt.toml"
  set -e
  run_lightning_ctl --check-local-storage \
    --backend local \
  ...
  set +e
  export GO_FAILPOINTS="github.com/wuhuizuo/tidb6/br/pkg/lightning/restore/FailIfIndexEngineImported=return(1)"
  run_lightning --backend local --enable-checkpoint=1 --log-file "$TEST_DIR/lightning-local.log" --config "tests/$TEST_NAME/$ckpt.toml"
  set -e
  run_lightning_ctl --check-local-storage \
    --backend local \
